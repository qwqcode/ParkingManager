...
public interface RecMapper {
    ...
    @Update("UPDATE `recs` SET `rec_pay_id` = #{rec_pay_id}, `rec_pay_at` = NOW() WHERE `id` = #{rec_id}")
    ...
    @Insert("INSERT INTO `rec_pays` (`car_id`, `rec_id`, `park_id`, `user_id`, `price`, `use_ticket_id`, `use_coupon_id`, `is_use_vip_card`) " +
            "VALUES (#{p.car_id}, #{p.rec_id}, #{p.park_id}, #{p.user_id}, #{p.price}, #{p.use_ticket_id}, #{p.use_coupon_id}, #{p.is_use_vip_card})")
    @Options(useGeneratedKeys = true, keyProperty = "id")
